...
namespace DDG
{
   ...
   Quaternion :: Quaternion( double s_, const Vector3D& v_ )
   // initializes with specified double(s) and imaginary (v) components
   : s( s_ ),
     v( v_ )
   {}
   ...
   Quaternion :: Quaternion( const Vector3D& v_ )
   ...
   const Quaternion& Quaternion :: operator=( const Vector3D& _v )
   ...
   Vector3D& Quaternion::im( void )
   // returns reference to imaginary part
   {
      return v;
   }
   
   const Vector3D& Quaternion::im( void ) const
