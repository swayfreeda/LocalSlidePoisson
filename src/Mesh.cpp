...
namespace DDG
{
    ...
    Mesh* Mesh::copyMesh()
    {
        ...
        for( HalfEdgeCIter he = this->halfedges.begin(); he != this->halfedges.end(); he++ )
            halfedgeOldToNew[ he ] = mesh->halfedges.insert( mesh->halfedges.end(), *he );
        
        mesh->vertices.clear();
        for(   VertexCIter  v =  this->vertices.begin();  v !=  this->vertices.end();  v++ )
            vertexOldToNew[ v  ] =  mesh->vertices.insert(  mesh->vertices.end(), *v  );
        
        mesh->edges.clear();
        for(     EdgeCIter  e =  this->edges.begin();  e !=     this->edges.end();  e++ )
            edgeOldToNew[ e  ] = mesh->edges.insert(     mesh->edges.end(), *e  );
        
        mesh->faces.clear();
        for(     FaceCIter  f =     this->faces.begin();  f !=     this->faces.end();  f++ )
            faceOldToNew[ f  ] =     mesh->faces.insert(     mesh->faces.end(), *f  );
        
        
        // "search and replace" old pointers with new ones
        for( HalfEdgeIter he = this->halfedges.begin(); he != this->halfedges.end(); he++ )
        {
            he->next   = halfedgeOldToNew[ he->next   ];
            he->flip   = halfedgeOldToNew[ he->flip   ];
            he->vertex =   vertexOldToNew[ he->vertex ];
        ...
        for( VertexIter v = this->vertices.begin(); v != this->vertices.end(); v++ ) v->he = halfedgeOldToNew[ v->he ];
        for(   EdgeIter e =    this->edges.begin(); e !=    this->edges.end(); e++ ) e->he = halfedgeOldToNew[ e->he ];
        for(   FaceIter f =    this->faces.begin(); f !=    this->faces.end(); f++ ) f->he = halfedgeOldToNew[ f->he ];
